...
export class PackageService {
  ...
  public static async findPackageById(packageId: string): Promise<Package> {
    try {
      const findPackage: Package | null = await PackageModel.findOne({
        _id: packageId,
      });
      if (!findPackage)
        throw new HttpException(
          400,
          `Package with ID: ${packageId} doesn't exist`
        );
      return findPackage;
    } catch (error) {
      throw new HttpException(
        500,
        `Error finding package with ID: ${packageId}`
      );
    }
  }

  public static async createPackage(packageData: Package): Promise<Package> {
    try {
      const createPackageData: Package = await PackageModel.create(packageData);
      return createPackageData;
    } catch (error) {
      throw new HttpException(500, `Error creating package ${error}`);
    }
  }
  ...
  public static async updatePackage(
    ...
    try {
      const packageData: Package | null = await PackageModel.findById(
        packageId
      );
      if (!packageData) {
      ...
      const updatedPackageData: Package | null =
      ...
      throw new HttpException(
        500,
        `Error updating package with ID: ${packageId}`
      );
    }
  }

  public static async deletePackage(packageId: string): Promise<Package> {
    try {
      const packageData: Package | null = await PackageModel.findById(
        packageId
      );
      ...
      const deletePackageById: Package | null =
